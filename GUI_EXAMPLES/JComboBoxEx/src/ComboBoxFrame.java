...
public class ComboBoxFrame extends JFrame
{
	...
	public ComboBoxFrame()
	{
		...
		imagesJComboBox.addItemListener(
			//similar to -> public class MyHandler implements ItemListener
				...
				public void itemStateChanged(ItemEvent event)
				{
					...
					if (event.getStateChange() == ItemEvent.SELECTED){
						label.setIcon(icons[imagesJComboBox.
						                    getSelectedIndex()]);
						/* For each item selected from a JComboBox , 
						 * another item is first deselected—so two 
						 * ItemEvents occur when an item is selected. 
						 * We wish to display only the icon for the 
						 * item the user just selected. 
						 * For this reason, line above determines whether 
						 * ItemEvent method getStateChange returns 
						 * ItemEvent.SELECTED
						 */
					}	
				}
